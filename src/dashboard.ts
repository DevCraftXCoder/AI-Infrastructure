...
<div id="tab-topology" style="display:none">
  <div class="toolbar"><div class="toolbar-right"><button class="btn ghost" onclick="loadTopology()">↺ Refresh</button></div></div>
...
var st={org:'',project:ALL,region:ALL,status:ALL,q:'',view:'comfortable',services:[],summary:{},lastLoad:0,logTab:'all',obsWindow:3600000,infraCost:'—'};
var allLogs=[];
...
function loadTopology(){var b=$('topologyBody');if(!b)return;var svcs=st.services;if(!svcs||!svcs.length){b.innerHTML='<div class="panel empty"><div>No services registered.</div><div class="hint">POST /api/control/services to register one.</div></div>';return}var byProj={};svcs.forEach(function(s){if(!byProj[s.project])byProj[s.project]=[];byProj[s.project].push(s)});b.innerHTML='';Object.keys(byProj).sort().forEach(function(proj){var g=document.createElement('div');g.className='panel';g.style.cssText='padding:16px;margin-bottom:12px';var hdr=document.createElement('div');hdr.className='section-header';var title=document.createElement('span');title.className='section-title';title.textContent=proj;hdr.appendChild(title);var cnt=document.createElement('span');cnt.className='badge';cnt.style.marginLeft='8px';cnt.textContent=byProj[proj].length+' svc';hdr.appendChild(cnt);g.appendChild(hdr);var row=document.createElement('div');row.style.cssText='display:flex;flex-wrap:wrap;gap:10px;align-items:center;margin-top:10px';byProj[proj].forEach(function(s,i){if(i>0){var a=document.createElement('span');a.style.cssText='color:var(--muted);font-size:16px;user-select:none';a.textContent='→';row.appendChild(a)}var node=document.createElement('div');node.className='panel';node.style.cssText='padding:10px 14px;cursor:pointer;min-width:120px;text-align:center;transition:border-color .15s';node.onmouseenter=function(){this.style.borderColor='var(--accent)'};node.onmouseleave=function(){this.style.borderColor=''};node.onclick=function(){openService(s.id)};node.innerHTML='<div style="font-size:12px;font-weight:600;margin-bottom:6px">'+s.name+'</div><span class="badge '+s.status+'">'+s.status+'</span><div style="font-size:10px;color:var(--muted);margin-top:5px">'+s.kind+(s.url?'<br><span style=\'color:var(--accent);font-size:9px\'>● public</span>':'<br><span style=\'color:var(--muted);font-size:9px\'>○ local</span>')+'</div>';row.appendChild(node)});g.appendChild(row);b.appendChild(g)})}
qsInit();load();
})();
</script>
</body>
