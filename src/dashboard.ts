...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .sidebar{width:var(--sidebar);min-width:var(--sidebar);background:var(--panel);border-right:1px solid var(--border);display:flex;flex-direction:column;overflow-y:auto}
  ...
  .topbar-sel{background:var(--panel2);color:var(--text);border:1px solid var(--border);border-radius:8px;padding:5px 10px;font:inherit;font-size:12px;cursor:pointer}
...
var gk=sessionStorage.getItem('gk')||'';
...
function promptKey(){var k=prompt('Enter GATEWAY_KEY:');if(!k)return;gk=k;sessionStorage.setItem('gk',k)}
...
function loadAccess(){var headers=gk?{'Authorization':'Bearer '+gk}:{};fetch('/api/control/access/session',{headers:headers}).then(function(r){return r.json()}).then(function(d){if(!d)return;txt($('aIdentity'),d.identity||'Unknown');txt($('aActorName'),d.actor||'—');txt($('aProvider'),d.auth_provider||'—');txt($('aRiskVal'),d.access_risk_score||'—');var roleEl=$('aRole');txt(roleEl,d.role||'viewer');roleEl.className='badge '+(d.role||'viewer');var mfaEl=$('aMfa');var mfaBanner=$('mfaBanner');if(d.mfa_status==='missing'){mfaEl.className='badge down';txt(mfaEl,'⚠ NO MFA SIGNAL');if(mfaBanner)mfaBanner.style.display=''}else if(d.mfa_status==='verified'){mfaEl.className='badge ok';txt(mfaEl,'✓ VERIFIED');if(mfaBanner)mfaBanner.style.display='none'}else{mfaEl.className='badge warn';txt(mfaEl,'UNKNOWN')}var expEl=$('aExpiry');if(d.session_expires_at){var diff=d.session_expires_at-Date.now();if(diff<0){expEl.style.color='var(--down)';txt(expEl,'Expired — '+fmt(d.session_expires_at))}else{expEl.style.color='';txt(expEl,fmt(d.session_expires_at))}}}).catch(function(){});renderPermMatrix()}
